...
export default function EditarIngredientePage() {
  const [ingredienteSelecionadoId, setIngredienteSelecionadoId] = useState<number | string | null>(null)
  ...
  async function pesquisarIngredientes(textoPesquisa: string) {
    ...
    if (error) {
      ...
      setIngredientesEncontrados([])
      setMensagem(`Erro ao pesquisar ingredientes: ${error.message}`)
      setAPesquisar(false)
      return
    }
  ...
  function carregarIngredienteNoFormulario(ingrediente: Ingrediente) {
    setIngredienteSelecionadoId(ingrediente.id)
    ...
    setMensagem(`Estás a editar o ingrediente "${ingrediente.nome}".`)
    window.scrollTo({ top: 0, behavior: 'smooth' })
  }

  function limparFormulario() {
    setIngredienteSelecionadoId(null)
    setNome('')
    setUnidadeBase('g')
    setPrecoUnitario('')
    setUnidadePreco('kg')
  ...
  function validarFormulario() {
    if (!ingredienteSelecionadoId) return 'Tens de selecionar um ingrediente para editar.'
  ...
  async function atualizarIngrediente() {
    ...
    const { data: existente, error: erroExistente } = await supabase
      ...
      .neq('id', ingredienteSelecionadoId)
      .maybeSingle()

    if (erroExistente) {
      console.log('Erro ao verificar nome existente:', erroExistente)
      setMensagem(`Erro ao verificar ingrediente existente: ${erroExistente.message}`)
      setAGuardar(false)
      return
    }

    if (existente) {
      setMensagem('Já existe outro ingrediente com esse nome.')
      setAGuardar(false)
      return
    }
    ...
    const { error } = await supabase
      ...
      .eq('id', ingredienteSelecionadoId)

    if (error) {
      console.log('Erro ao atualizar ingrediente:', error)
      setMensagem(`Erro ao atualizar o ingrediente: ${error.message}`)
      setAGuardar(false)
      return
    }
    ...
    if (pesquisa.trim()) {
      await pesquisarIngredientes(pesquisa.trim())
    }
    ...
    <main className="min-h-screen bg-white text-black p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <section className="border rounded p-6 bg-gray-50 mb-8">
          <h2 className="text-2xl font-bold mb-4">Pesquisar ingrediente</h2>
                  ...
                  >
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        {ingrediente.categoria || 'Sem categoria'}
                      </p>
                    </div>

                    <button
                      type="button"
                      onClick={() => carregarIngredienteNoFormulario(ingrediente)}
        ...
        <section className="border rounded p-6 bg-gray-50">
          ...
          {!ingredienteSelecionadoId && (
            <div className="mb-4 p-4 rounded bg-white border">
              <p className="text-sm text-gray-700">
                Pesquisa e seleciona primeiro um ingrediente para poderes editá-lo.
              </p>
            </div>
          )}
          ...
          <div className="flex gap-3 mt-6 flex-wrap">
            <button
              type="button"
              onClick={atualizarIngrediente}
              disabled={aGuardar || !ingredienteSelecionadoId}
              className="text-white px-6 py-3 rounded font-medium disabled:opacity-50"
              style={{ backgroundColor: '#80c944' }}
            >
