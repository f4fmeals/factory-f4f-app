...
export default function Home() {
  ...
  async function ativarProducao(producao: ProducaoSemanal) {
    ...
    if (producaoSelecionada?.id === producao.id) {
      setProducaoSelecionada({ ...producao, estado: 'ativo' })
    }
        ...
        componentesIngredientes.filter((ci) => Number(ci.componente_id) === Number(pratoComponente.componente_id)).forEach((compIng) => {
          ...
          if (!compras[categoria][chaveIngrediente]) {
            compras[categoria][chaveIngrediente] = {
              ...
              nome: nomeIngrediente, quantidade: 0, unidade, preco, unidade_preco: unidadePreco, custo_total: 0,
              nome_fornecedor: nomeFornecedor, quantidade_embalagem: quantidadeEmbalagem, unidade_embalagem: unidadeEmbalagem, pratosTalho: {},
            }
          }
  ...
  const listaPreparacao = useMemo(() => {
    ...
    return Object.values(agrupado).map((ingredienteGrupo) => ({
      chave: ingredienteGrupo.chave, ingredienteNome: ingredienteGrupo.ingredienteNome, quantidadeTotalIngrediente: ingredienteGrupo.quantidadeTotalIngrediente, unidade: ingredienteGrupo.unidade,
  ...
  const listaFinalizacao = useMemo(() => {
    ...
    })).filter((bloco) => bloco.tarefas.length > 0).sort((a, b) => a.pratoNome.localeCompare(b.pratoNome))
  ...
  function renderProducao() {
    return (
      <>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '2px' }}>
                    ...
                    {p.estado === 'ativo' && (
                      <span style={{ backgroundColor: '#80c944', color: '#fff', fontSize: '10px', fontWeight: '600', padding: '2px 8px', borderRadius: '99px', whiteSpace: 'nowrap' }}>
                        ✓ Ativo
                      </span>
                    )}
                  </div>
        ...
        {/* Detalhes da produção selecionada — botão Ativar aparece aqui */}
        {producaoSelecionada && (
          <div style={{ marginTop: '24px', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '1.25rem', background: '#f9fafb' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
              ...
              {producaoSelecionada.estado === 'ativo' ? (
                <span style={{ backgroundColor: '#80c944', color: '#fff', fontSize: '12px', fontWeight: '600', padding: '5px 14px', borderRadius: '6px' }}>
                  ✓ Plano ativo
                </span>
              ) : (
                <button
                  onClick={() => ativarProducao(producaoSelecionada)}
                  disabled={aAtivarProducaoId === producaoSelecionada.id}
  ...
  // ... renderCompras, renderPreparacao, renderConfeccao, renderFinalizacao, renderEmbalamento, renderAreaImpressao permanecem exatamente iguais ao original ...
      ...
      <div className="print-area">
        <div className="print-page">
                ...
                return (
                  <div key={categoria} className="print-block">
                    ...
                    <table className="print-table">
                      <thead><tr><th>Ingrediente</th><th>Fornecedor</th><th>Quantidade</th><th>Preço</th>{ehTalho && <th>Quantidade por prato</th>}</tr></thead>
                      <tbody>
                        {Object.values(itens).map((info) => (
          ...
          {secaoExportar === 'finalizacao' && (
            <div className="print-section">
              {listaFinalizacao.length === 0 ? <p>Sem dados de finalização.</p> : listaFinalizacao.map((bloco) => (
                <div key={bloco.chave} className="print-block">
                ...
                <div key={grupo.chave} className="print-block">
                  ...
                  {grupo.tamanhos.map((linha) => (
                    <div key={linha.chave} className="print-subblock">
                      ...
                      <p>SKU: {linha.sku}</p><p>Quantidade: {linha.quantidade}</p>
                      <table className="print-table">
